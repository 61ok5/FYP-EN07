...
if(int(sys.argv[1]) == 0):
    df = db_getall()
    dump(df, '/home/user/api/src/util/ml_preloads/df.joblib')
    ...
    dump(cosine_sim, '/home/user/api/src/util/ml_preloads/cosine_sim.joblib')

    indices = pd.Series(df.index, index=df['id']).drop_duplicates()
    dump(indices, '/home/user/api/src/util/ml_preloads/index.joblib')
    ...
    def get_recommendations(id, cosine_sim=cosine_sim):
        ...
        return df[['id','title','image_750x422']].iloc[course_indices]
    ...
    df_clean_db = df[['id','title','price','has_test','has_certificate','has_mobile','instructional_level'
                ,'instructor','i_category','p_category','ps_category','caption','context', 'num_subscribers']].copy()

    df_clean_db['caption'] = df_clean_db['caption'].apply(str)

    if(int(sys.argv[3])==1):
        df_clean_db.to_sql('course_clean', con=engine, if_exists='replace')
    ...
    dump(cosine_sim2, '/home/user/api/src/util/ml_preloads/cosine_sim2.joblib')

    # Reset index of our main DataFrame and construct reverse mapping as before
    df = df.reset_index()
    indices = pd.Series(df.index, index=df['id'])
    dump(indices, '/home/user/api/src/util/ml_preloads/index2.joblib')
else:
    # df = db_getall()
    df = load('/home/user/api/src/util/ml_preloads/df.joblib')
    # indices = pd.Series(df.index, index=df['title']).drop_duplicates()
    if(int(sys.argv[2])==0):
        indices = load('/home/user/api/src/util/ml_preloads/index.joblib')
    if(int(sys.argv[2])==1):
        indices = load('/home/user/api/src/util/ml_preloads/index2.joblib')
...
if(int(sys.argv[2])==0):
    # indices = load('/home/user/api/src/util/ml_preloads/index.joblib')
    print(get_recommendations(int(sys.argv[4])).to_json(orient = 'records'))
if(int(sys.argv[2])==1):
    # indices = load('/home/user/api/src/util/ml_preloads/index2.joblib')
    print(get_recommendations(int(sys.argv[4]), cosine_sim2).to_json(orient = 'records'))
