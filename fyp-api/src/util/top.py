...
def course_rank():
    ...
    m= df['num_reviews'].quantile(0.98)
    m

    q_course = df.copy().loc[df['num_reviews'] >= m]
    q_course.shape

    def weighted_rating(x, m=m, C=c):
        v = x['num_reviews']
        R = x['rating']
        # Calculation based on the IMDB formula / Bayesian estimate
    ...
    return q_course[['title', 'id', 'num_reviews', 'rating', 'score', 'image_750x422']]
...
def result_simple(title):
    get_recommendations(title)

def result_genres(title):
    get_recommendations(title, cosine_sim2)
